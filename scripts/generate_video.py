...
def build_task(seed: int, challenge_type: int, family_id: str = "cf_autopilot"):
    ...
    return task_for_seed_and_type(
        SIM_DT, seed=int(seed), challenge_type=int(challenge_type), family_id=family_id,
    )



def _load_seed_jobs(seed_file: Path, family_id: str = "cf_autopilot") -> List[VideoJob]:
    ...
    type_seeds = _load_type_seeds(Path(seed_file), family_id=family_id)
    jobs: List[VideoJob] = []
...
def _link_workspace(extracted: Path) -> None:
    ...
    # Mirrors the container's own layout for models that hardcode it. Outside a container
    # that path is usually unwritable, and a missing mirror only matters to such a model,
    # so a refusal here must not end the render.
    ws = Path(os.environ.get("SWARM_VIDEO_WORKSPACE", "/workspace/submission"))
    try:
        ws.mkdir(parents=True, exist_ok=True)
    except OSError as exc:
        print(
            f"[video] cannot mirror the submission into {ws} ({exc.strerror}); continuing. "
            f"Set SWARM_VIDEO_WORKSPACE to a writable path if the model needs that layout.",
            flush=True,
        )
        return
...
class _FlightRecorder:
    ...
    def start(self, env: object, goal: Tuple[float, float, float], horizon: float) -> None:
        ...
        self._total_frames = int(float(horizon) * self._fps)
    ...
    def finish(self, success: bool, sim_time_sec: float) -> List[VideoResult]:
        ...
        for mode in self._modes:
            out_path = self._out_paths.get(mode)
            if out_path is None:
                # start() never ran, so the container died before the env existed; report
                # that rather than a KeyError on the mode name.
                continue
...
def record_flight(
    ...
    *,
    ...
    cli = getattr(env, "CLIENT", 0)
    # Width is per-family: search-and-rescue carries a 6th RGB-request value, and a swarm
    # family expects one row per drone, so neither may be assumed from the autopilot shape.
    n_drones = int(getattr(env, "NUM_DRONES", 1))
    act_dim = int(env.action_space.shape[-1])
    if n_drones > 1:
        act_lo, act_hi = env.action_space.low, env.action_space.high
    else:
        act_lo, act_hi = env.action_space.low.flatten(), env.action_space.high.flatten()
...
def _resolve_jobs(args: argparse.Namespace) -> List[VideoJob]:
    if args.seed_file is not None:
        ...
        return _load_seed_jobs(args.seed_file, family_id=args.family_id)
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    if args.replay_actions is not None and args.backend != "local":
        raise ValueError(
            "--replay-actions requires --backend local; the benchmark backend always "
            "runs the policy live"
        )
