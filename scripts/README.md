...
- `generate_video.py`
  - Renders mp4 flight videos for a model + seed, or a saved benchmark seed file. Also reachable as `swarm video`. Accepts `--family-id` (default `cf_autopilot`) and `--backend {local,benchmark}` (local fast replay vs. the exact benchmark Docker/RPC replay), e.g. `python3 scripts/generate_video.py --model model/UID_178.zip --seed 42 --type 7 --family-id cf_office_interceptor --mode chase`.
