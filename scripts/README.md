...
- `generate_video.py`
  - Renders mp4 flight videos for a model + seed, or a saved benchmark seed file. Also reachable as `swarm video`. Accepts `--family-id` (default `cf_autopilot`) and `--backend {local,benchmark}` (local fast replay vs. the exact benchmark Docker/RPC replay), e.g. `python3 scripts/generate_video.py --model Submission/submission.zip --seed 42 --type 7 --family-id cf_office_interceptor --mode chase`.
- `stress_benchmark_compare.py`
  ...
  - Example smoke test:
    - `python3 scripts/stress_benchmark_compare.py --model Submission/submission.zip --seed-count 100 --repetitions 5 --workers 6 --relax-timeouts --rpc-verbosity low`
  - Example full run:
    - `python3 scripts/stress_benchmark_compare.py --model Submission/submission.zip --seed-count 1000 --repetitions 5 --workers 6 --relax-timeouts --rpc-verbosity low`
- `test_timings.py`
