...
def generate_road_positions(rng: SeededRNG, min_spacing: int, target_area: int, tile_size: float = None, map_size: float = None):
    """Generate road positions using specified tile_size for grid spacing."""
    if tile_size is None:
        tile_size = TILE_SIZE  # Default to constant
        
    actual_map_size = map_size if map_size is not None else MAP_SIZE
    ...
    num_tiles_f = round(actual_map_size / tile_size)
...
def generate_road_tiles(v_pos, h_pos, rng: SeededRNG = None, max_block_size=60, tile_size: float = None, difficulty: int = 1, map_size: float = None) -> Tuple[List[RoadTile], set, list, list, list, list]:
    ...
    num_tiles = round(actual_map_size / tile_size)
...
def generate_buildings(blocks: List[Block], rng: SeededRNG, city_type: int = 2, b_margin=4, difficulty: int = 1) -> List[Building]:
    ...
    def process_block(block, road_margin=4, center_gap=1.5, block_tier='commercial'):
        ...
        # We evaluate placement locally per block; road separations prevent cross-block overlap.
