...
# ──────────────────────────────────────────────────────────────────────────────
# Existing imports / functions (kept exactly as‑is)
# ──────────────────────────────────────────────────────────────────────────────
...
async def dendrite_with_retries(
    ...
    timeout: float,
    retries=1,
) -> _List[TaskSynapse | SetOperatorEndpointSynapse | None] | None:
    """
    (…existing code left untouched…)
    """
    res: _List[TaskSynapse | SetOperatorEndpointSynapse | None] = [None] * len(axons)
    idx = list(range(len(axons)))
    axons = axons.copy()

    try:
        for attempt in range(retries):
            responses: _List[TaskSynapse | SetOperatorEndpointSynapse] = await dendrite(
