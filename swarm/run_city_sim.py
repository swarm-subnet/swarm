...
OBJ_DIR = os.path.join(BASE_DIR, "assets")
SCALE_FACTOR = 5.0
MODEL_BASE_SIZE = 1.8 
...
def get_asset_for_zone(category, lot_w, lot_d, city_type, block_style=None):
    ...
    if category == 'tower':
        if 'tower' not in ASSET_SPECS:
            print("DEBUG: No tower category in ASSET_SPECS!")
            return None
        ...
        if not skyscrapers:
            print("DEBUG: No skyscraper assets found!")
            return None
    ...
    if city_type == 3 and effective_category == 'tower':
        ...
        if skyscraper_only:
            ...
            print(f"DEBUG: Hard Mode - No skyscraper asset for tower. SKIPPING.")
            return None
    ...
    if city_type == 3 and effective_category == 'tower':
        print(f"DEBUG: Hard Mode - No skyscraper dimension match for {lot_w}x{lot_d}. Skipping.")
        return None
...
def get_asset_path(name):
    ...
    if variants: return os.path.join(OBJ_DIR, random.choice(variants))
    return None
...
def spawn_asset_exact(path, x, y, z, rotation_deg, scale_vec, rgbaColor=None):
    ...
    try:
        ...
        # Only print for Houses to reduce noise, or first few
        if 'suburban' in path: 
            print(f"DEBUG: Spawned House ID {body_id} at {x:.1f},{y:.1f} | Scale: {scale_vec[0]:.2f} | AABB Z: {min_aabb[2]:.2f} to {max_aabb[2]:.2f}")
    except:
        pass
...
def spawn_asset_with_random_color(path, x, y, z, rotation_deg, scale_vec, warm_mode=False):
    ...
    try:
        ...
        print(f"DEBUG: Color variant failed ({e}), using original")
        return spawn_asset_exact(path, x, y, z, rotation_deg, scale_vec)
...
def load_asset_simple(name, x, y, z, rotation_deg):
    ...
    # CARS: Removed +90 check. Assuming standard rotation (0=North/Forward).
...
# ... (skipping to offset) ...
    ...
    cars_spawned = 0
    lane_offset_factor = 0.22 # Increased to separating lanes better (was 0.17, originally 0.23)
...
# (OFFSET ADJUSTMENT IN REGENERATE FUNCTION)
# Scanning down to finding regenerate_city_with_settings to apply offset change...
# Actually I need to apply edits in two places. 
# 1. load_asset_simple (Rotation)
# 2. regenerate_city_with_settings (Offset)

# Let's do load_asset_simple first in this chunk.
...
def regenerate_city_with_settings(target_area, lamp_freq, city_type, density=2, max_block_size=80, difficulty=1, road_width=10.0, color_mode=1):
    ...
    print(f"DEBUG: Regenerating with Width={road_width}m (Scale={road_scale:.2f}), min_area={target_area}")
    ...
    print("DEBUG: Calling city_gen.generate_city...")
    try:
    ...
    for b in city_data['buildings']:
        ...
        if difficulty == 3 and b.type != 'tower':
            print(f"DEBUG: Hard Mode - Skipping non-tower building type: {b.type}")
            continue
        ...
        if not path:
            if difficulty == 3:
                print(f"DEBUG: Hard Mode - No skyscraper match for tower {rect.w}x{rect.h}. SKIPPING.")
                continue
            print(f"DEBUG: Zone match failed for {b.type} {rect.w}x{rect.h}. Using fallback.")
    ...
    print(f"Placed {building_count} buildings.")
    
    # Calculate and visualize empty spaces in block centers
    # Buildings are on the perimeter, empty space is in the center
    building_depth = 12  # Increased margin to avoid overlap with buildings
    for block in city_data['blocks']:
        if block.too_small: continue
        
        br = block.rect
        
        # Empty center = block minus building perimeter on all sides
        empty_x = br.x + building_depth
        empty_y = br.y + building_depth
        empty_w = br.w - (building_depth * 2)
        empty_h = br.h - (building_depth * 2)
        
        # Only show if there's meaningful space
        if empty_w > 3 and empty_h > 3:
            area = empty_w * empty_h
            z = 0.5
            
            # Color based on size level
            if area < 200:
                color = [1, 0.3, 0.3]  # Red - Small (أشجار فقط)
                label = f"S:{int(area)}m²"
            elif area < 500:
                color = [1, 1, 0.3]  # Yellow - Medium (حديقة صغيرة)
                label = f"M:{int(area)}m²"
            else:
                color = [0.3, 1, 0.3]  # Green - Large (حديقة كبيرة/ملعب)
                label = f"L:{int(area)}m²"
            
            # Draw rectangle (DISABLED PER USER REQUEST)
            # p.addUserDebugLine([empty_x, empty_y, z], [empty_x+empty_w, empty_y, z], color, 2)
            # p.addUserDebugLine([empty_x+empty_w, empty_y, z], [empty_x+empty_w, empty_y+empty_h, z], color, 2)
            # p.addUserDebugLine([empty_x+empty_w, empty_y+empty_h, z], [empty_x, empty_y+empty_h, z], color, 2)
            # p.addUserDebugLine([empty_x, empty_y+empty_h, z], [empty_x, empty_y, z], color, 2)
            ...
            if random.random() < 0.35:
                ...
                if tile.type == 'straight_v':
                    ...
                    if direction == 1: # Heading East (+X)
                        ...
                        # Wait, PyBullet Obj Rotation:
                        # Usually 0 is Forward?
                        # Let's align with Road rotation logic. 
                        # Road 90 = Horizontal.
                        # If we assume 0 = North.
                        # East is 270 (-90) or 90?
                        # In my Traffic Light logic:
                        # "Right side (South): Faces North (0) -> Corrected to 180 (South)"
                        # Let's try standard compass: N=0, E=270, S=180, W=90.
                    else: # Heading West (-X)
                         # Right side = North (+Y)
                         car_y = cy + offset
    ...
    return {'buildings': building_count, 'houses': sum(1 for b in city_data['buildings'] if b.type == 'house'), 
            'apts': sum(1 for b in city_data['buildings'] if b.type == 'apt'), 
            'towers': sum(1 for b in city_data['buildings'] if b.type == 'tower'), 
...
def run_simulation():
    ...
    while p.isConnected():
        ...
        if keys.get(112, 0) & p.KEY_WAS_TRIGGERED:
            ...
            print(f"DEBUG: Toggling Wireframe -> {wireframe_on}")
        ...
        if curr_gen > last_gen:
            ...
            if difficulty == 3:
                r_width = 6
                print("DEBUG: Hard Mode Active -> Forcing Road Width to 6m & Urban Type")
                c_type = 3
