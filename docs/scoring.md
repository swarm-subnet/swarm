...
Gate 2 — IMPROVEMENT (the weighted score must clear the threshold)
    weighted_score  >=  current_champion.benchmark_score  +  0.015
```

If no champion exists yet, Gate 2 falls back to `weighted_score >= 0.01` (no `+0.015` added).

The `+0.015` floor is the minimum improvement a new model must show against the current champion.
...
| **Improvement floor** | The minimum gap (`+0.015`) a challenger's weighted score must show above the current champion's benchmark score. |
